// CK_DX_03_03_TextureBox.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//

#include "framework.h"
#include "CK_DX_03_03_TextureBox.h"
	...
	_In_ int       nCmdShow)
{
	...
	CKDX_TextureBox theApp(hInstance);
...
CKDX_TextureBox::CKDX_TextureBox(HINSTANCE hInstance)
...
{
	mMainWndCaption = L"CK_DX_TextureBox Demo";
...
CKDX_TextureBox::~CKDX_TextureBox()
{
	ReleaseCOM(mBoxVB);
	ReleaseCOM(mBoxIB);
...
void CKDX_TextureBox::UpdateScene(float dt)
{
...
void CKDX_TextureBox::DrawScene()
{
...
void CKDX_TextureBox::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void CKDX_TextureBox::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void CKDX_TextureBox::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void CKDX_TextureBox::BuildGeometryBuffers()
{
